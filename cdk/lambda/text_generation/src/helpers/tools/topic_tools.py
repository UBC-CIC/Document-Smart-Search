...
class TopicTools:
    ...
    def __init__(
        ...
        html_index_name: str,  # Added html_index_name parameter
        region: str,
        conn 
    ):
        self.opensearch_client = opensearch_client
        self.topic_index_name = topic_index_name
        self.html_index_name = html_index_name  # Store html_index_name
        self.region = region
        self.conn = conn 
    ...
    def _count_topic_documents(self, topic_name: str) -> int:
        """Count total number of documents related to a topic."""
        count_query = f"""
        SELECT COUNT(*)
        FROM documents d
        INNER JOIN documents_topics dt
          ON d.html_url = dt.html_url
        WHERE dt.topic_name = '{topic_name}'
    ...
    def get_combined_topics(self) -> str:
        """
        Combine topics into a single text string with document counts.
        """
        items_by_name = self.get_all_items()
        combined_items = ""
        for item_name, docs in items_by_name.items():
            # Get document count for this topic
            doc_count = self._count_topic_documents(item_name)
    ...
    def topic_related_documents_tool(self, topic_name: str) -> str:
        ...
        # Query to count total documents for this topic
        count_query = f"""
        SELECT COUNT(*) 
        ...
        try:
            ...
            error_output = f"Error retrieving documents for topic '{topic_name}': {str(e)}"
            return json.dumps({
                "output": error_output,
                "metadata": {
                    "description": f"LLM categorized documents that relate to topic: {topic_name}",
