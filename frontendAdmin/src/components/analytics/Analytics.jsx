...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}admin/analytics`, {
          method: "GET",
          headers: {
            Authorization: token,
  ...
  const processedData = uniqueMonths.map((month) => {
    ...
    return {
      month: `${monthNames[Number.parseInt(monthNum, 10) - 1]} ${year}`,
  ...
  return (
    <main className="flex-1 p-3 sm:p-6 w-full overflow-x-hidden">
      ...
      >
        ...
        >
          <XAxis dataKey="month" label={{ value: "Month", position: "bottom", offset: 0 }} tick={{ fontSize: 10 }} />
          <YAxis
            label={{
              value: "Unique Users",
              angle: -90,
            ...
            tick={{ fontSize: 10 }}
          />
          <Line type="monotone" dataKey="unique_users" stroke="var(--color-unique_users)" strokeWidth={2} dot={true} />
          <ChartTooltip content={<ChartTooltipContent />} />
        </LineChart>
      </ChartContainer>

      <div className="text-lg mb-4 mt-8">User Engagement by Month</div>
      <ChartContainer
        config={{
          public: {
      ...
      <div className="mb-8 mt-8 space-y-6 mr-0 sm:mr-12">
        <div>
        ...
        <div className="space-y-4 mx-4 sm:mx-12">
          {displayedFeedback.map((feedback) => (
            <div key={feedback.userType} className="space-y-2">
              ...
              <Progress value={((feedback.score - 1) / 4) * 100} className="h-2 bg-adminAccent [&>div]:bg-adminMain" />
            </div>
          ))}
        </div>
      </div>
