  ...
  return (
    <div className="space-y-3">
      {validDocuments.map((doc) => (
        <div
          key={doc.id}
          className="border-b dark:border-gray-700 pb-3 last:border-b-0 last:pb-0"
        >
          <Link
            href={`/documents/${doc.id}`}
            className="block hover:bg-gray-50 dark:hover:bg-gray-700 -mx-4 px-4 py-2 rounded"
          >
            ...
            {doc.csasEvent && (
              <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                <span className="font-medium">CSAS Event:</span> {doc.csasEvent}{" "}
                {doc.csasYear && `(${doc.csasYear})`}
              </div>
            )}
