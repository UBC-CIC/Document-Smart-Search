...
const relevancyExplanationHybridSearch =
  "Relevancy here represents a hybrid score combining semantic similarity (70%) and keyword matching (30%) based on a given query. Since semantic scoring is relative to all documents in the database, a high percentage doesn't always guarantee relevance to your specific question.";
const relevancyExplanationTopicsOrMandates =
  "Relevancy here represents the LLM's confidence in the categorization of the document based on the topics or mandates. A high percentage indicates a strong match to the expected categories, but does not guarantee relevance to your specific question.";
const relevancyExplanationDerivedTopics =
  "Relevancy here represents how semantically relevant the derived topics are to the original document. A high percentage does not guarantee relevance to your specific question.";
...
export function CitationsSidebar({
  ...
  const hasAnySources = (): boolean => {
    ...
    return activeToolsUsed.tools_and_sources.some(
      (tool) => tool.sources && tool.sources.length > 0
    );
  };
  ...
  const getRelevancyExplanation = (toolName: string): string => {
    if (
      toolName.includes("Mandate Related Documents") ||
      toolName.includes("Topic Related Documents")
    ) {
  ...
  const findToolForSource = (source: Source): string | null => {
    ...
    for (const tool of activeToolsUsed.tools_and_sources) {
      if (
        tool.sources &&
        tool.sources.some(
          (s) => s.document_id === source.document_id && s.url === source.url
        )
      ) {
                    ...
                    >
                      ...
                      {tool.sources && tool.sources.length > 0 && (
                        <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                          {tool.sources.length} source
                          {tool.sources.length > 1 ? "s" : ""}
                        </p>
                      )}
                    </div>
                          ...
                          <div className="space-y-3">
                            {tool.sources
                              ...
                              .map((source, sourceIndex) => {
                                const explanation = getRelevancyExplanation(
                                  tool.tool_name
                                );
